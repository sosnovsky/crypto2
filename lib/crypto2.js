...
const aes256cbcDecrypt = function (text, password, callback) {
  //const decipher = crypto.createDecipher('aes-256-cbc', password); //ORIGINAL
  const decipher = crypto.createDecipher('aes-128-gcm', password); //Added by David
...
const rsaEncrypt = function (text, publicKey, options, callback) {
  process.nextTick(() => {
...
const rsaDecrypt = function (text, privateKey, options, callback) {
  process.nextTick(() => {
...
const sha256Sign = function (text, privateKey, callback) {
  ...
  processStream(sign, text, { from: 'utf8', to: 'utf8' }, err => {
    ...
    const signature = sign.sign(privateKey, 'base64'); // David change 'hex' to 'base64'
...
const sha256Verify = function (text, publicKey, signature, callback) {
  ...
  processStream(verify, text, { from: 'utf8', to: 'utf8' }, err => {
    ...
    const isSignatureValid = verify.verify(publicKey, signature, 'base64'); // David change 'hex' to 'base64'
